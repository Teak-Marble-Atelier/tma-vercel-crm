// src/public/SignPage.tsx
// PUBLIC customer surface at /sign/:token. No CRM session — reads and signs
// purely through the envelopes-public Edge Function, which authorizes by
// token hash. Never imports the app's supabase client (that carries CRM auth).
// Same fetch/state pattern as QuotePage.tsx.

import { useEffect, useState } from "react";
import { useParams } from "react-router-dom";

const FN = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/envelopes-public`;

interface SigningOrderRow { role_label: string; name: string; status: string; routing_order: number }
interface EnvelopeView {
  envelope_number: string; title: string; doc_type: string;
  role_label: string; name: string;
  document_url: string | null; source_sha256: string;
  signing_order: SigningOrderRow[];
  consent_text: string;
}
...
export default function SignPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (res.status === 425) { setMsg(body.message); setState("waiting"); return; }
        if (res.status === 410 || res.status === 404) { setMsg(body.message ?? "This signing link is no longer available."); setState("gone"); return; }
        if (body.status === "already_signed") { setMsg(`Signed ${new Date(body.signed_at).toLocaleString()}.`); setState("signed"); return; }
        setEnv(body); setState("active");
      } catch { setMsg("Something went wrong loading this document."); setState("gone"); }
    })();
  }, [token]);
  ...
  async function sign() {
    ...
    try {
      ...
      const body = await res.json();
      setSubmitting(false);
      if (!res.ok) { setError(body.message ?? body.error ?? "Something went wrong — please try again."); return; }
      setMsg(body.message); setState("done");
    } catch {
      setSubmitting(false);
    ...
    <div style={S.page}>
      <div style={S.card}>
        ...
        {env!.document_url
          ...
          : <div style={{ ...S.viewer, display: "flex", alignItems: "center", justifyContent: "center", color: "#888" }}>Document unavailable</div>}
